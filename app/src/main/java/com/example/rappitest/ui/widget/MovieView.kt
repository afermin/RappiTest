package com.example.rappitest.ui.widget
...
class MovieView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
    ...
    private var mVideoResourcePath: String? = null

    var title: String = ""
    var path: String
        get() = this.toString()
        set(value) {
            setVideoResourcePath(value)
        }
    ...
    init {
        ...
        setVideoResourcePath(a.getString(R.styleable.MovieView_android_path))
     ...
     * @return Path of the video resource.
     */
    fun getVideoResourceId(): String? = mVideoResourcePath
    ...
    /**
     * Sets the raw resource Path of video to play.

     * @param id The raw resource ID.
     */
    fun setVideoResourcePath(path: String) {
        if (path!!.isEmpty() || path == mVideoResourcePath) {
            return
        }
        mVideoResourcePath = path
    ...
    public fun startVideo() {
        mMediaPlayer?.let { player ->
            player.reset()
            try {
                player.setDataSource(mVideoResourcePath)
